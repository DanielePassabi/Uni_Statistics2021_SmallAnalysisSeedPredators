...
There is *no substantial difference* between the reference value (10 meters) and 25 meters.
...
Since we are working with a qualitative variable, it's easy to plot our regression line.
...
Our `ANOVA` results seems to be confirmed. We can see that there are significant differences between most of the months and the month taken as a reference (April). The only one that has a high p-value is March, and that may make sense since March and April are consecutive months. 

We can confirm this even further, using `TukeyHSD`, a function that computes the Honest significance differences and works on the output of `aov`.
...
We can see how the last two models are the ones with better results, according to the metrics used.

---

### Experiments with Models 

Now, we would like to fit a more complex model, using all the variables that are reasonable to use.
...
models = c("distance", "species", "cumulative time", "month", "complex v2")
AIC = c(AIC(model_dist), AIC(model_species), AIC(model_time), AIC(model_month), AIC(model_exp_2))
BIC = c(BIC(model_dist), BIC(model_species), BIC(model_time), BIC(model_month), BIC(model_exp_2))
